...
{
	// This should all be done dynamically based on config files

	vector<featureIndex> tcpFeatures;
	tcpFeatures.push_back(IP_TRAFFIC_DISTRIBUTION);
	tcpFeatures.push_back(PORT_TRAFFIC_DISTRIBUTION);
	tcpFeatures.push_back(DISTINCT_PORTS);
	tcpFeatures.push_back(DISTINCT_IPS);
